...
*****************************************************************************/
/**
@defgroup scope_module       Functions provided to allow access to the scope mode data.
# Introduction
...
The firmware contains a series of scope mode multiplexers which select which channels the data comes from and from wich points within the channels. The streams can be set to take data from multiple points within 
one channel or to take data from 1 point across multiple channels, or some mixture of this.
@n
Once the scope mode multiplexer have been set up, the system, is started using {@link xsp3_system_start()}. This starts the system and records the data into DRAM on the FEM.
...
The layout of the data is chosen to suit the DMAs in the firmware. For XSPRESS3 it is in 2 blocks to match the use of 2 DMA streams. 
The functions here {@link xsp3_scope_mod_get_nstreams()}, {@link xsp3_scope_mod_get_ptr()} and {@link xsp3_scope_mod_get_inc() } should be used to determine 16bit pointers top access it.
...
However, since only 1 process can have full control of the XSPRESS3/4 and own the UDP ports to run the histogramming threads, it is possible for other processes to just access scope mode data from the module.
...
These can be set by a GUI (such as imgd) in the data module and then copied to the hardware using {@link xsp3_scope_settings_from_mod()} called in the process with direct control on XSPRESS3/4 before calling {@link xsp3_system_start()}
...
Whenever the scope mode data is displayed these field should be used to interpret the data.
## XSPRESS3 streams.
...
### 4 stream mode
    ...
    Currently there is no advantage to use 4 bit, though could mux in 4 global digital signals. @n
### 8 Stream
    ...
    7 analogue + 16 bit digital = 2 bit digial on 7 @n
    7 analogue + 16 bit digital = all digial on 7 @n
...
* The layout of the {@link xsp3_scope_module_head} and {@link xsp4_scope_module_head} are aligned at first so it is usual to read particularly num_cards and num_t.
*/
...
typedef struct xsp4_scope_data_module
{
	struct xsp4_scope_module_head
	{
		...
		int num_t;					//!< Number of time points to use.
		int current_lwords_per_card;	//!< Amemory layout with smaller scope to use part of the module.
		struct
		{
...
extern const char *xsp4_scope_name_s0to5[16];
...
extern const char *xsp4_scope_alt_names_s0to5[XSP4_SCOPE_NUM_SRC0TO5][16];
...
extern const char *xsp3m_scope_name_s0to4[16];
extern const char *xsp3m_scope_name_s5[16];
extern const char *xsp3m_scope_alt_names_s0to4[XSP4_SCOPE_NUM_SRC0TO5][16];
