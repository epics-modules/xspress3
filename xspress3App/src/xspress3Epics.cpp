...
Xspress3::Xspress3(const char *portName, int numChannels)
    : ADDriver(portName, numChannels, NUM_DRIVER_PARAMS, -1, -1, INTERFACE_MASK, INTERRUPT_MASK, ASYN_CANBLOCK | ASYN_MULTIDEVICE, 1, 0, 0)
    , debug_(1)
    , numChannels_(numChannels)
    , simTest_(1)
    , baseIP_("127.0.0.1")
...
asynStatus Xspress3::erase(bool sendFrame)
{
    ...
    if ((status = checkConnected()) == asynSuccess) {
        ...
        xsp3_curr_frames += 1;
        if (xsp3_used_frames == 0) {
            xsp3_curr_frames = 1;
        }
        if (xsp3_used_frames > xsp3_curr_frames) {
            xsp3_curr_frames = xsp3_used_frames;
        }
        if (xsp3_curr_frames > xsp3_time_frames) {
            xsp3_curr_frames = xsp3_time_frames;
        }
...
asynStatus Xspress3::setupITFG(void)
{
    ...
    if (trigger_mode == mbboTriggerINTERNAL_ && xsp3->has_itfg(xsp3_handle_, 0) > 0) {
        getIntegerParam(ADNumImages, &num_frames);
        getDoubleParam(ADAcquireTime, &exposureTime);
    ...
    if (trigger_mode == mbboTriggerTTLVETO_ && (xsp3->has_itfg(xsp3_handle_, 0) > 0) && ppt) {
...
asynStatus Xspress3::mapTriggerMode(int mode, int invert_f0, int invert_veto, int debounce, int *apiMode)
{
    ...
    if (invert_f0)
        *apiMode |= XSP3_GLOB_TIMA_F0_INV;
    if (invert_veto)
        *apiMode |= XSP3_GLOB_TIMA_VETO_INV;
    if (debounce)
        *apiMode |= XSP3_GLOB_TIMA_DEBOUNCE(debounce);
...
asynStatus Xspress3::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
    ...
    } else if (function == xsp3TriggerModeParam || function == xsp3InvertF0Param || function == xsp3InvertVetoParam || function == xsp3DebounceParam) {
        if ((status = checkConnected()) == asynSuccess) {
            int trigger_mode, invert_f0, invert_veto, debounce;
            ...
            if (function == xsp3TriggerModeParam)
                trigger_mode = value;
            else if (function == xsp3InvertF0Param)
                invert_f0 = value;
            else if (function == xsp3InvertVetoParam)
                invert_veto = value;
            else
                debounce = value;
